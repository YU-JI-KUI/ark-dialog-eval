...
| 形态 | `SKILL.md`(灵魂)+ `scripts/`(确定性脚本)+ `reference/`(各 BU rubric) | REST API + React 看板 + PostgreSQL 落盘 |
...
证券和寿险共用**流水线骨架**(会话重组 → Judge → 漏斗聚合 → 洞察 → 建议;不做样本过滤、答案原文进 LLM 不做代码解析),不共用的是**领域知识与提示词**。领域配置抽成可插拔的 `BUConfig`(`backend/app/core/bu/`);提示词与业务分类统一放 `backend/app/core/eval/prompts/`。

> Java 类比:`BUConfig` 是领域**策略对象**(Strategy Pattern),引擎是上下文,运行时按上传选择的 BU 注入对应策略。

### BU 的可配项(代码 + 提示词两处)

**代码侧**(`BUConfig`,`backend/app/core/bu/<bu>.py`):
...
| `sample_calib` / `sample_prod` | 内置样例文件名 | `securities_dialog_calib_2000.xlsx` |

**提示词侧**(`backend/app/core/eval/prompts/<bu>/`,改文件即生效,见该目录 `README.md`):

| 文件 | 作用 |
|------|------|
| `categories.json` | 业务分类清单 + 定义(喂模型时渲染成 markdown 表格;前端用它计数/列名) |
| `judge_system.md` | 该 BU 的评测人设 |
| `task_*.md` | 各评测维度的判定规则(BU 专属覆盖,缺则回退 `_default/`) |

> 提示词与判定规则不再写死在代码里,全部外置到 `prompts/`,内网改文件、重启即生效。
...
1. **加配置**:复制 `backend/app/core/bu/securities.py` → `property_insurance.py`,填 `code="property"`、`name`、`dispatch_aliases`、mock 规则、样例文件名。
2. **加业务分类**:新建 `backend/app/core/eval/prompts/property/categories.json`(业务分类 + 定义)。
3. **登记一行**:在 `backend/app/core/bu/registry.py` 的 `_REGISTRY` 里加 `PROPERTY.code: PROPERTY`。
4. **(可选)专属提示词**:在 `prompts/property/` 下放 `judge_system.md` / `task_*.md`;不放则自动回退 `_default/`。skill 路线另加 `reference/property_rubric.md`。

**引擎、Judge、指标、建议、前端选择器全部零改动**——它们都通过 `bu` 参数 + `prompts/<bu>/` 拿领域知识。

> 寿险业务分类目前是**按常见业务设计的合理占位**(保单查询 / 理赔 / 缴费 / 保全 …)。拿到真实分类文档后,改 `prompts/life/categories.json` 即可,无需动代码。
...
│    会话重组 · 漏斗聚合 · 指标κ/F1 · 不一致导出 · 报告/明细导出 │
│                          │                                 │
...
日志是逐轮的平铺行。流水线按「应用会话ID」分组、按「客户咨询轮次」排序,把每一轮还原成一条样本,并拼上**前文上下文**(`context`)。关键:`context` 不只含前文用户问,还含**前文 AI 的回答原文**(不做代码解析,交给 LLM 读懂)。
...
前置:`uv`(Python 包管理)、`node ≥ 18`、**PostgreSQL**(任务/结果持久化)。**Judge 用 mock 后端无需任何模型,开箱即跑。**

```bash
# 0) 配置数据库:复制 .env.sample → .env,填 DB_* 连接信息(默认连本地 postgres)
cd backend && cp .env.sample .env   # 按需改 DB_HOST/DB_NAME/DB_USER/DB_PASSWORD

# 1) 后端依赖(含 sqlalchemy + psycopg2)
uv sync
...
# 4) 一键起前后端(回到项目根;首次启动自动建表)
cd .. && ./dev.sh
```

> 数据库表(`t_eval_task` / `t_eval_task_row`)在后端首次启动时自动创建(`store.init_db()`),无需手动建表。需要一个能连的空库即可。
...
cd backend
# 对证券校准集跑评测并打印 κ/F1
uv run python -m scripts.run_calibration data/sample/securities_dialog_calib_2000.xlsx
...
| ② 流水线 | `uv run python -m scripts.run_calibration data/sample/securities_dialog_calib_2000.xlsx` | mock 全链路接线 | 流水线编排问题 |
...
> **大数据量保障**:任务逐批落盘 PostgreSQL,跑一半中断可**断点续跑**(`POST /api/eval/tasks/{id}/resume`,跳过已完成行)。所有样本一律走真实大模型精判,不做快层粗筛——要的是评测准确性,夜间用自有推理服务跑,机器空闲不算浪费。
...
平安 One 会:读 `SKILL.md` → 加载对应 `categories/<bu>.json` + `reference/<bu>_rubric.md` → 调 `scripts/prepare.py`(会话重组,不过滤、不解析答案,答案原文进上下文)→ 逐条按四条铁律评测 →(有金标则)调 `scripts/metrics.py` 算 κ → 按 `reference/report_template.md` 出报告。脚本依赖 `pandas`、`openpyxl`(prepare)、`scikit-learn`(metrics),内网走私服安装。真实日志「分发BU」列值非中文时,用 `prepare.py --dispatch-alias 证券 --dispatch-alias PA_SEC` 传入。
...
├── backend/
│   ├── app/
│   │   ├── core/bu/                 # ⭐ BU 领域知识层(代码侧:dispatch_aliases / mock 规则 / 样例名)
│   │   │   ├── base.py                 BUConfig + matches_dispatch + load_categories(读 prompts)
│   │   │   ├── securities.py           证券 BU
│   │   │   ├── life_insurance.py       寿险 BU(占位待替换)
│   │   │   └── registry.py             BU 注册表(新增 BU 登记一行)
│   │   ├── core/eval/               # 评测引擎(纯逻辑,与模型/BU 无关)
│   │   │   ├── prompt_loader.py        ⭐ 提示词统一加载(load_prompt / load_bu_prompt:BU优先、_default回退)
│   │   │   ├── prompts/                ⭐ 所有提示词 + 业务分类,统一管理(见该目录 README.md)
│   │   │   │   ├── judge_user.md           Judge 共用骨架(占位符装配)
│   │   │   │   ├── _default/*.md           通用人设 / 4 个评测维度 / 优化建议
│   │   │   │   ├── securities/             证券专属:categories.json + judge_system.md + task_*.md
│   │   │   │   └── life/                   寿险专属:categories.json + judge_system.md
│   │   │   ├── judge.py                Judge 消息构造(填充外置模板)+ 输出解析
│   │   │   ├── pipeline.py             列解析 / 会话重组 / 模式判定 / dispatched_to_bu(不过滤、不解析答案)
│   │   │   ├── metrics.py              准召F1 + Cohen's κ + 混淆矩阵
│   │   │   └── advisor.py              优化建议(填外置模板 + 规则兜底;脏 JSON 容错解析)
│   │   ├── core/llm/                # 模型调用层(唯一依赖内网)
│   │   │   ├── signature.py            平安双签名(RSA-SHA256 + HMAC-SHA1)
│   │   │   ├── pingan_client.py        平安大模型客户端
│   │   │   ├── mock_judge.py           规则桩 Judge(无模型时端到端跑通)
│   │   │   └── judge_runner.py         调度入口 judge_one(mock/pingan)+ 建议生成
│   │   ├── services/                # 任务编排
│   │   │   ├── evaluator.py            双模式评测 + BU 分发漏斗(四象限场景)+ 业务洞察 + 断点续跑
│   │   │   ├── task_manager.py         任务管理 + 导出(报告 / 逐条明细 / 不一致 case)
│   │   │   └── store.py               PostgreSQL 持久化(SQLAlchemy,任务 + 逐条结果)
│   │   ├── api/routes.py            # REST 接口(upload/sample/tasks/result/export*/meta)
│   │   ├── config.py               # 配置(环境变量 / .env;含 LOG_LEVEL)
│   │   └── main.py                 # FastAPI 入口(含前端 dist 托管)
│   ├── scripts/                    # 造样例 / 内网试水 / 命令行校准
│   ├── tests/                      # 单元 / 契约 / 分层 / 容错测试(uv run pytest)
│   └── .env.sample
├── frontend/                       # React + Vite + Tailwind v4 + Recharts
│   ├── src/components/             # 上传/进度/指标卡/洞察表/建议卡/明细表/详情抽屉/历史记录面板
│   ├── src/utils/format.js         # 统一时间格式 yyyy-MM-dd HH:mm:ss
│   └── dist/                       # 构建产物(后端托管;外网构建后带进内网)
├── skill/dialog-eval/              # ⭐ Skill 路线(给平安 One)
│   ├── SKILL.md                       灵魂:四条铁律 + 三维度漏斗 + 输出契约
│   ├── ROADMAP.md / README.md         落地路线图 / 使用说明
│   ├── categories/<bu>.json           业务分类清单(与后端 prompts 同源)
│   ├── scripts/                       与后端同源的确定性脚本(prepare / metrics)
│   └── reference/                     各 BU 评判细则 + 报告模板(纯文本,事实源)
├── start.sh / dev.sh / build.sh    # 内网一键托管 / 本地分离开发 / 仅构建前端
└── README.md
```
...
```bash
cd backend && uv run pytest -q     # 43 passed
```

覆盖(`tests/` 7 个文件 43 个用例):

- **列解析**精确匹配优先(防「答案」误命中「标准问答案」这类含子串列的回归)、**会话重组**(context 带前文 AI 答原文)、**模式自动判定**(空标注列不算金标)。
- **指标**计算与混淆矩阵方向、**Mock Judge** 规则、**建议解析容错**(模型吐脏 JSON 也能救回大部分建议)。
- **提示词分层**(`<bu>/` 优先、`_default/` 回退)、**业务分类读取**(读 business_type 不读旧字段)、**分发四象限场景**(正常 / 该拒未拒 / 该分未分,覆盖「都不该且没分=正常」反例)。
...
| 层 | 技术 |
|----|------|
| 后端 | FastAPI · uv · pandas · scikit-learn · httpx · pycryptodome / cryptography(双签名) · PostgreSQL(SQLAlchemy + psycopg2) |
