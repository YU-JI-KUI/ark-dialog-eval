...
3. **事实不可验证转人工**:没有知识库,绝不让模型瞎判业务事实(`factual_verifiable=false`、`factual_correct=null`)。
...
| **事实正确性不判** | 无知识库,涉及金额 / 条件 / 条款对错的一律 `factual_correct=null` 并转人工(铁律三) | 接入业务知识库后才可放开,当前刻意不碰 |
